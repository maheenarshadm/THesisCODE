...
5. **Final-dataset coverage capped far below archive coverage** — not a bug, a structural mismatch between how DynaMOSA's own population behaves (spreads into specialists) and how the final dataset was being picked (one individual from the last generation). *Fixed*: merge-the-archive (§8). **17-21/151 → 81/151, matching archive coverage exactly.**
6. **Provable infeasibility detection**, so "unsolved" stops being one undifferentiated bucket. *Fixed*: three-valued constant-folding (§3.2). **53/151 (35.1%) proven mathematically unsatisfiable; 81/81 genuinely solvable rules covered.**
...
7. **A small number of rules, in more than one case study, that are provably infeasible by construction** at the DMN-model level itself (a self-comparison, a grounding conflict) — findings about the source models, very likely authoring artifacts, not gaps in this pipeline.
