        ...
        if current_table:
            ...
            if m:
                ...
                if ctype in ('index',):
                    ...
                    # older explicit-type DSL: t.column :name, :type, options...
                    # COL_LINE already consumed "col" as the column name; the real
                    # type is the next :symbol token inside `rest`.
                    type_m = re.match(r'^,\s*[:"\']([\w]+)["\']?', rest)
                    real_type = type_m.group(1) if type_m else 'string'
                    tables[current_table]['columns'][col] = {'type': real_type, 'null_false': 'null: false' in rest}
                elif ctype == 'change':
                    # t.change :name, :type, options... -- alters an EXISTING column's
                    # type in place; must not be treated as adding a new column.
                    type_m = re.match(r'^,\s*[:"\']([\w]+)["\']?', rest)
                    real_type = type_m.group(1) if type_m else None
                    if real_type and col in tables[current_table]['columns']:
                        tables[current_table]['columns'][col]['type'] = real_type
                    elif real_type:
                        # column not seen yet in this parse order (e.g. type-only
                        # migration ordering quirk) -- record it so the type is at
                        # least captured rather than silently dropped.
                        tables[current_table]['columns'][col] = {'type': real_type, 'null_false': False}
                elif ctype == 'remove':
                    # t.remove :col1, :col2, ... -- deletes column(s), does not add one.
                    removed_names = [col] + re.findall(r'[:"\']([\w]+)["\']?', rest)
                    for rc in removed_names:
                        tables[current_table]['columns'].pop(rc, None)
                else:
                    null_false = 'null: false' in rest
