...
## Implementing `exists`-kind filter support (2026-09-13) -- the feasible half of two architectural gaps, and three real bugs found stacking merge-time offset fixes

Asked for a feasibility read on §13.48's two remaining architectural gaps before writing any code: the old/new-row-disambiguation gap (high cost, only 2 records, needs a genuinely new per-role focal concept) vs. the `exists`-kind filter gap (more feasible -- `derived_aggregate`'s own `filter_text`/`_mechanical_filter_predicate`/`_row_from_filter_conjuncts` machinery already exists and is proven, `exists` just never used it; caveat: FLEX2's own `courseOfferedInFollowingSemesters` wouldn't benefit either way, since it needs ordering, not equality). Chose to implement the `exists`-kind option.

Built in four layers (`compile_constraints.py`'s `classify_derived` parses an `exists`-kind raw schema field's column list into a real `filter_text`; `candidate.py`'s `derive_value`/`build_seed_candidate` and `mutation.py`'s `_apply_row_count_mutation` all reuse the same mechanical filter-predicate/row-construction machinery `derived_aggregate` already proved out), each verified independently -- degrading to the exact old behavior for every filter-less `exists` leaf (the overwhelming majority), confirmed via the unchanged `candidate.py` sweep count (264/291) and a diff of `compiled_constraints.json` scoped to exactly the 3 jBilling records that actually gained a filter.

Running jBilling end to end then surfaced three further bugs, all in `dynamosa.py`'s `merge_archive_candidate`, none assumed -- each found by re-testing, one of them (the worst) only because this session re-verifies EVERY case study after any shared-code change, not just the one being worked on:

1. **A within-record PK collision** (`Currency Exchange Rate Source::Rule_2` legitimately owns two `CURRENCY_EXCHANGE` rows, both independently repaired to `id=1`) -- fixed via `used_key_values` dedup tracking in `get_copy`, bumping a colliding post-offset value by +1 until free.
2. **The dedup fix bumped a legitimately-shared FK column** (`currency_id`, meant to be IDENTICAL across `Rule_2`'s own two rows -- same currency, different entity context) -- fixed by scoping the dedup bump to genuine PK/UNIQUE columns only (`_own_unique_columns_for`), never FK.
3. **That fix's flattening of COMPOSITE keys broke FLEX2 far worse than the bug it fixed.** `COURSE_REGISTRATION`'s composite PK/unique-index columns (`ROLL_NO`, `SEM_ID`, ...) are SUPPOSED to repeat identically across a student's own many `derived_aggregate` rows -- flattening the keyset into a per-column set and dedup-bumping any of them corrupted 930 legitimate repeats, collapsing FLEX2's own archive-matching coverage from 81/151 to a VERIFIED 56/151 (25 objectives). Fixed by restricting the dedup bump to single-column keysets only (`_own_solo_unique_columns_for`) -- every real collision found so far (`CURRENCY_EXCHANGE.id`, `BASE_USER.id`, `PURCHASE_ORDER.id`) is a bare `id` PK, so this still catches all of them.
4. **A business-value column (`entity_id`) that isn't a declared key at all still needed offsetting**, since `_row_from_filter_conjuncts` copies a scenario placeholder's value into it exactly like a real key column. A pure value-based fix reintroduced bug 3's failure mode; a pure column-name-based fix corrupted a DIFFERENT leaf's own literal `entity_id = 0` on the same table. Fixed by combining both: `_filter_columns_needing_offset` maps each filter-bound column to its placeholder(s) structurally, and `get_copy` additionally checks, per row, that the row's own current value matches the record's own scenario value for that placeholder before offsetting it.

**Verified end to end, all four case studies re-run after every fix**: FLEX2 81/151 (zero regressions, restored after the temporary 56/151 dip), Spree 22/27 (unaffected throughout), jBilling 32/42 → **33/42** (`Rule_2` now genuinely solved via its own distinct filter, matching archive coverage exactly, zero regressions). A pre-existing, unrelated `like`-kind crash was found sanity-checking OpenMRS (never run through this pipeline before) -- confirmed present identically before this section's own changes, so flagged honestly as out of scope rather than silently fixed. Full trace, including the exact bug-by-bug reasoning: `docs/generationalgorithmdesign.md` §13.49.

## Running the pipeline against OpenMRS for the first time (2026-09-13) -- four real bugs, three of them general regression-machinery gaps

Ran the pipeline against OpenMRS, the fourth and last case study, for the first time. Crashed immediately (`re.search`'s own pattern argument wasn't a string) -- found and fixed four real bugs, each re-verified against all FOUR case studies, not just OpenMRS, before moving to the next:

1. **A hardcoded generic placeholder corrupted a sibling leaf's own required type.** `null_check`'s own mutation logic restores a nulled column to "any non-null value" via a bare `1` -- wrong the instant a SIBLING leaf on the same record reads that exact column with type-specific semantics (`Identifier Format Validity`'s own `formatSet`/`identifierMatchesFormat` share `patient_identifier_type.format`, one as a bare not-null flag, the other as a regex STRING). Fixed by making the restore type-aware (reusing `_placeholder_for_column_type`, the same mechanism `_repair_row` already uses). A parallel seed-time fix was tried and reverted -- it dropped the full-corpus sweep 264→249, since an UNRELATED record's own ordering comparison depends on the OLD, type-blind consistency between a `null_check` seed and a `schema_column` sibling seed. Also fixed the FALSE side: a NOT-NULL column's own "blank" state must write `''`, never a literal `None` (confirmed by a real `NOT NULL constraint failed`, matching the ground truth's own "IS NULL OR TRIM = ''" reading).
2. **FK enforcement during INSERT is fundamentally insert-order-sensitive**, and OpenMRS's real schema has a genuine 28-table FK cycle -- 254 of its own rows were flagged as FK failures purely because of which table got inserted first, none of them ACTUALLY dangling. Fixed by inserting with FK enforcement off (order-independent), then running `PRAGMA foreign_key_check` once against the fully-populated database -- a real, order-independent, per-row check. Also fixed a related reporting bug this exposed: `topological_table_order`'s own informational cycle notes were being counted as validation failures, so a dataset that validated perfectly clean was still reported `ok=False`.
3. **Merge-time offsetting broke a THIRD row-to-row value correspondence**, distinct from both jBilling bugs in the `exists`-kind filter section above: a `join_lookup` leaf finds its joined-to row by matching a VALUE column (not the real PK) against the FK's own value -- a documented, narrow-scope trick `build_seed_candidate`/mutation both keep in sync by construction. Merge-time offsetting shifted the FK column (a declared key) without shifting the OTHER row's matching value column (never itself a key), breaking the join. Fixed with a new structural helper, `_join_lookup_pairs`, mirroring the `exists`-kind fix's own pattern but table-scoped (the correspondence is inherently cross-table).
