...
**Not built yet, and deliberately out of scope for this pass**: the DynaMOSA search loop itself (§6.4) -- crossover/mutation operators over this genome representation, population management, non-dominated sorting with the dynamic DRD-edge-gated objective activation §6.4 specifies, and the constraint-satisfaction terms (NOT NULL/UNIQUE/FK/CHECK, §6.3's "combining with database integrity constraints" section) this fitness function doesn't yet sum in alongside the DMN-derived terms.
