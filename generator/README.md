...
**Scope, stated plainly**: FK distance is only computable for FLEX2 right now -- OpenMRS/Spree/jBilling's own parsers still discard per-column FK detail the same way FLEX2's did before today, a shared gap across all three, named here rather than silently left implicit. `fk_distance` raises `FitnessEvaluationError` rather than silently returning 0 when a case study's schema entry has no `fk_columns` at all, so this limitation surfaces loudly if hit rather than passing as a false "no violation."
